...
const useTodoActions = ({ todo, onUpdate, onDelete }) => {
  ...
  const [updateTodo] = useTodoUpdateMutation({onSuccess: () => {}, onError: () => {}});
  const [deleteTodo] = useTodoUpdateMutation({onSuccess: () => {}, onError: () => {}});
    ...
    action => {
      switch (action.type) {
        ...
        case "TOGGLE": {
          ...
          updateTodo({ ...todo, completed: !todo.completed }).then((updatedTodo) => {
            setViewState(prev => ({
              ...prev,
              activeAction: null,
              loading: false
